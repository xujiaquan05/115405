...
#
# 為什麼這裡「刻意」留在記憶體，而爬取鎖與頻率限制改存資料庫？
# 因為三者的失效方式不同：
# - 爬取鎖放記憶體是「正確性」問題：多 worker 會同時跑兩批爬取。
# - 頻率限制放記憶體是「安全性」問題：N 個 worker 等於把限制放寬 N 倍。
# - cache 放記憶體只是「命中率」問題：每個 worker 各自暖機，
#   答案永遠正確，只是重算的次數多一些。
#
# 而且把 cache 搬進資料庫會反過來增加資料庫負載——正是 cache 想減輕的東西。
# 真的要跨 worker 共用 cache，正確作法是 Redis；在還沒有 Redis 之前，
# 留在記憶體是比較好的選擇。
CACHE_STORE = {}
