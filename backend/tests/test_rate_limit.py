...
class TestRateLimiter:
    def test_allows_up_to_limit(self):
        ...
        for _ in range(3):
            limiter(request)  # không được raise
    ...
    def test_different_ips_counted_separately(self):
        ...
        limiter(make_request(ip="2.2.2.2"))  # IP khác, không bị chặn

    def test_forwarded_for_takes_priority(self):
        # Trên Render, mọi request đều đến từ proxy IP —
        # phải đếm theo X-Forwarded-For, không theo IP proxy.
        limiter = RateLimiter(max_requests=1, window_seconds=60)
    ...
    def test_window_expiry_allows_again(self, monkeypatch):
        ...
        fake_time[0] += 61  # hết window

        limiter(request)  # được phép lại, không raise
