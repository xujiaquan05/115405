...
const features = [
  ...
  },
  {
    icon: "☺",
    title: "AI 情緒分析",
    desc: "以 Gemini 逐篇判讀正面 / 中性 / 負面，連留言也逐則評分——輿情的關鍵往往藏在討論串裡。",
  },
  {
    icon: "⌕",
    title: "關鍵字輿情追蹤",
  ...
  },
  {
    icon: "⚠",
    title: "主動輿情預警",
    desc: "為品牌或療程設定監控關鍵字，負面聲量一旦超過門檻就自動示警，不必天天盯著看板。",
  },
  {
    icon: "✦",
    title: "可執行行銷洞察",
  ...
  },
  {
    icon: "⇩",
    title: "報表匯出",
    desc: "將篩選後的文章與分析結果匯出成 Excel，方便納入簡報或做進一步統計。",
  },
];

const steps = [
  { no: "1", title: "爬取資料", desc: "自動收集 PTT、Dcard、Mobile01、Threads 四大平台的最新討論。" },
  { no: "2", title: "過濾清洗", desc: "去除公告與交易雜訊，擷取正文與留言，只留下真實的消費討論。" },
  { no: "3", title: "AI 分析", desc: "逐篇與逐則留言判讀情緒，彙整關鍵字並計算聲量趨勢。" },
  { no: "4", title: "輸出洞察", desc: "產出圖表、行銷建議與風險預警，必要時可匯出報表。" },
];
...
const faqs = [
  ...
  {
    q: "情緒分析是怎麼判讀的？準確嗎？",
    a: "由 Gemini 逐篇判讀正面 / 中性 / 負面，而且我們有實際量測：抽樣 100 篇文章逐篇標註後比對，隨機樣本的整體正確率為 90.7%，「負面」的召回率 0.897 —— 真正的負面討論有近九成被抓到。這是輿情監控最在意的指標，漏掉負評的代價遠高於誤報。最常見的誤判是把「語氣像抱怨的提問」判成負面，方向上偏保守。標註是由另一個語言模型閱讀全文後完成，尚未經人工複核，因此這組數字應視為參考而非定論。尚未評分的文章會暫以推文數作為過渡指標，每篇的 AI 覆蓋率也會顯示，方便你評估分數可信度。",
  },
  {
    q: "需要登入才能使用嗎？",
